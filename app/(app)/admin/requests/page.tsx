      ...
      <main className="mx-auto max-w-md space-y-4 px-5 py-5">
        ...
        {data === undefined ? (
          <div className="space-y-3">
            {[0, 1].map((i) => (
              <div key={i} className="h-32 animate-pulse rounded-3xl bg-line/60" />
            ))}
          </div>
              ...
              {pending.map((r) => (
                ...
                >
                  ...
                  <a
                    ...
                    className="tabular mt-2 inline-flex min-h-11 items-center text-sm font-semibold text-accent-deep underline-offset-4 hover:underline"
                  >
        ...
        {decided.length > 0 ? (
          <section className="overflow-hidden rounded-3xl border border-line bg-surface">
            <div className="border-b border-line px-4 py-3">
              <p className="text-sm font-semibold text-ink">Already decided</p>
            </div>
                ...
                <div key={String(r.requestId)} className="px-4 py-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span
                      className={
                        r.status === "approved"
                          ? "shrink-0 rounded-full border border-accent/30 bg-accent-soft px-2.5 py-0.5 text-xs font-semibold text-accent-deep"
                          : "shrink-0 rounded-full border border-line px-2.5 py-0.5 text-xs font-semibold text-faint"
                      }
                    >
                      {r.status === "approved"
                        ? r.used
                  ...
                  <p className="tabular mt-0.5 text-xs text-faint">{r.phone}</p>
                  {r.status === "dismissed" ? (
                    <button
                      type="button"
                      disabled={busyId === r.requestId}
                      onClick={() => void decide("approve", r.requestId)}
                      className="mt-1.5 min-h-11 text-xs font-semibold text-accent-deep underline-offset-4 hover:underline disabled:opacity-50"
                    >
